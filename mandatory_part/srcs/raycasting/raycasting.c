...
/*   Updated: 2022/11/02 19:56:13 by jdubilla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_results_on_screen(t_ray *ray)
{
	...
	i = 0;
	j = 0;
	color_ceiling = create_rgb(ray->data->c.r, ray->data->c.g, ray->data->c.b);
	color_floor = create_rgb(ray->data->f.r, ray->data->f.g, ray->data->f.b);
	while (j < HEIGHT)
	{
		i = 0;
		while (i < WIDTH)
